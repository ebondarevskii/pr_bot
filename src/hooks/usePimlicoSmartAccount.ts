...
export const usePimlicoSmartAccount = () => {
  ...
  const [smartAccount, setSmartAccount] = useState<PimlicoSmartAccount | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  const createSmartAccount = useCallback(async (): Promise<PimlicoSmartAccount | null> => {
    try {
      setIsLoading(true);
      setError(null);
        ...
        const tempClient = createSmartAccountClient({
          ...
          userOperation: {
            estimateFeesPerGas: async () => {
              return (await smartAccount.pimlicoClient.getUserOperationGasPrice()).fast
            },
  ...
  const getUserOperationStatus = useCallback(async (userOperationHash: string) => {
    ...
    try {
      ...
      return await pimlicoClient.getUserOperationStatus({ userOperationHash });
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get user operation status';
      setError(errorMessage);
      return null;
    }
  }, [smartAccount]);

  const waitForUserOperationReceipt = useCallback(async (userOperationHash: string) => {
    if (!smartAccount?.client) {
      setError('Smart account client not available');
      return null;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to wait for user operation receipt';
      setError(errorMessage);
      return null;
    }
  }, [smartAccount]);
  ...
  const getUSDCBalance = useCallback(async (): Promise<bigint | null> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get USDC balance';
      setError(errorMessage);
      return null;
    }
  }, [smartAccount]);
  ...
  const estimateTransactionCost = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to estimate transaction cost';
      setError(errorMessage);
      return null;
    }
